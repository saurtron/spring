...
#include "Sim/Units/BehaviourAI/BuilderCmdBehaviourAI.h"
#include "Sim/Units/Scripts/CobInstance.h"
...
void CBuilderCmdBehaviour::PreInit(const UnitLoadParams& params)
{
...
bool CBuilderCmdBehaviour::CanAssistUnit(const CUnit* u, const UnitDef* def) const
{
	RECOIL_DETAILED_TRACY_ZONE;
...
bool CBuilderCmdBehaviour::CanRepairUnit(const CUnit* u) const
{
	RECOIL_DETAILED_TRACY_ZONE;
	const auto& unitDef = owner->unitDef;
...
bool CBuilderCmdBehaviour::UpdateTerraform(const Command&)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
bool CBuilderCmdBehaviour::AssistTerraform(const Command&)
{
	RECOIL_DETAILED_TRACY_ZONE;
	const auto inBuildStance = owner->inBuildStance;
	CBuilderCmdBehaviour* helpTerraformee = helpTerraform;
...
bool CBuilderCmdBehaviour::UpdateBuild(const Command& fCommand)
{
	RECOIL_DETAILED_TRACY_ZONE;
	const auto inBuildStance = owner->inBuildStance;
	const auto& unitDef = owner->unitDef;
...
bool CBuilderCmdBehaviour::UpdateReclaim(const Command& fCommand)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
bool CBuilderCmdBehaviour::UpdateResurrect(const Command& fCommand)
{
	RECOIL_DETAILED_TRACY_ZONE;
	const auto& pos = owner->pos;
	const auto team = owner->team;
	...
	if (!curResurrectee->deleteMe) {
		...
		for (const int resurrecterID: CBuilderCaches::resurrecters) {
			...
			CBuilderCmdBehaviour* resurrecterAI = resurrecter->GetBehaviour<CBuilderCmdBehaviour>();
...
bool CBuilderCmdBehaviour::UpdateCapture(const Command& fCommand)
{
	RECOIL_DETAILED_TRACY_ZONE;
	const auto inBuildStance = owner->inBuildStance;
	const auto& pos = owner->pos;
...
void CBuilderCmdBehaviour::UpdatePre()
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void CBuilderCmdBehaviour::SlowUpdate()
{
  	RECOIL_DETAILED_TRACY_ZONE;
	if (terraforming) {
...
void CBuilderCmdBehaviour::SetReclaimTarget(CSolidObject* target)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void CBuilderCmdBehaviour::SetResurrectTarget(CFeature* target)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void CBuilderCmdBehaviour::SetCaptureTarget(CUnit* target)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void CBuilderCmdBehaviour::StartRestore(float3 centerPos, float radius)
{
	RECOIL_DETAILED_TRACY_ZONE;
	StopBuild(false);
...
void CBuilderCmdBehaviour::StopBuild(bool callScript)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
bool CBuilderCmdBehaviour::StartBuild(BuildInfo& buildInfo, CFeature*& feature, bool& inWaitStance, bool& limitReached)
{
	RECOIL_DETAILED_TRACY_ZONE;
	const auto allyteam = owner->allyteam;
	const auto team = owner->team;
...
float CBuilderCmdBehaviour::CalculateBuildTerraformCost(BuildInfo& buildInfo)
{
	RECOIL_DETAILED_TRACY_ZONE;
...
void CBuilderCmdBehaviour::DependentDied(CObject* o)
{
	RECOIL_DETAILED_TRACY_ZONE;
	if (o == curReclaim) {
		curReclaim = nullptr;
...
{
	/* TODO REFACTOR: also at BuilderBehaviour */
